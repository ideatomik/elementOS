...
function init() {
  loadJSON(function(response) {
    ...
    // number
      ...
      newEl.appendChild(newNumSpan);
    
      newNumSpan.classList.add('number');
   ...
   // symbol
      ...
      newEl.appendChild(newSymSpan);

      newSymSpan.classList.add('symbol');
      newEl.dataset.symbol = element.symbol;

      if (element.number >= 95 && element.number <= 118)
      {
        newSymSpan.classList.add('synthetic');
        newBox.classList.add('syn');
      }
    ...
    // name
      ...
      newEl.appendChild(newNameSpan);

      if (element.number == 59 || element.number == 104 || element.number == 118)
      {
        newNameSpan.classList.add('nameExtra');
      }
      else {
    ...
    // info1
      ...
      newEl.appendChild(newInfSpan);

      newInfSpan.classList.add('info1');
    ...
    const testthing = document.createElement("div");
    newEl.appendChild(testthing);
    testthing.classList.add('testsquare');
    const testbutton = document.createElement("button");
    testthing.appendChild(testbutton);
    testbutton.classList.add('testbutton', 'cleanbutton');
    testbutton.appendChild(document.createTextNode("+"));
    //testbutton.addEventListener("click", PopulateDiagram(newEl.dataset.electron_configuration,newEl.dataset.number));
    
    testbutton.addEventListener("click", function(){
      PopulateDiagram(newEl.dataset.electron_configuration,newEl.dataset.number, newEl.dataset.name);
      RaiseOVerlay();
    });
        } 
    });
